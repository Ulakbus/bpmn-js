...
function BpmnSnapping(eventBus, canvas, bpmnRules) {
  ...
  eventBus.on('resize.start', 1500, function(event) {
    ...
    if (is(shape, 'bpmn:Lane')) {
      context.minBounds = computeLaneMinBounds(shape, context.direction);
    }
...
var asTRBL = require('diagram-js/lib/layout/LayoutUtil').asTRBL,
    asBounds = require('diagram-js/lib/layout/LayoutUtil').asBounds;
...
function computeMinBounds(elements, padding) {
  function nonNull(e) {
    return !!e;
  }

  var getBBoxFn = getChildBoxFn(padding);

  var childrenBoxes = elements.map(getBBoxFn).filter(nonNull);
...
function getChildBoxFn(padding) {

  return function generatedGetChildBox(child) {
    ...
    return addPadding(child, padding);
  };
}

function computeParticipantMinBounds(participantShape) {
  return computeMinBounds(participantShape.children, PARTICIPANT_PADDING);
}


var assign = require('lodash/object/assign');

var getContainments = require('../modeling/LaneUtil').getContainments;
...
function computeLaneMinBounds(laneShape, resizeDirection) {

  var containments = getContainments(laneShape);

  var laneTRBL = asTRBL(laneShape);

  var minBoundsTRBL = assign({ }, laneTRBL);


  var participantBounds = computeMinBounds(containments.parentChildren, PARTICIPANT_PADDING),
      laneRefsBounds = computeMinBounds(containments.laneRefs, LANE_PADDING);

  var participantTRBL = participantBounds && asTRBL(participantBounds),
      laneRefsTRBL = laneRefsBounds && asTRBL(laneRefsBounds);

  if (/s/.test(resizeDirection)) {
    minBoundsTRBL.top = laneTRBL.top;
    minBoundsTRBL.bottom = laneRefsTRBL ? laneRefsTRBL.bottom : (laneTRBL.top + LANE_MIN_DIMENSIONS.height);
  }

  if (/n/.test(resizeDirection)) {
    minBoundsTRBL.top = laneRefsTRBL ? laneRefsTRBL.top : (laneTRBL.bottom - LANE_MIN_DIMENSIONS.height);
    minBoundsTRBL.bottom = laneTRBL.bottom;
  }

  if (/e/.test(resizeDirection)) {
    minBoundsTRBL.left = laneTRBL.left;
    minBoundsTRBL.right = participantTRBL ? participantTRBL.right : laneTRBL.left + LANE_MIN_DIMENSIONS.width;
  }

  if (/w/.test(resizeDirection)) {
    minBoundsTRBL.left = participantTRBL ? participantTRBL.left : laneTRBL.right - LANE_MIN_DIMENSIONS.height;
    minBoundsTRBL.right = laneTRBL.right;
  }

  return asBounds(minBoundsTRBL);
}